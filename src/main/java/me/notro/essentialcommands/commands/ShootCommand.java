...
public class ShootCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!sender.hasPermission("essentials.shoot")) {
            sender.sendMessage(MessageUtility.fixColor(MessageUtility.NO_PERMISSION.getDefaultMessage()));
            return false;
        }

        if (args.length < 1) {
            sender.sendMessage(MessageUtility.fixColor("&cUsage&7: &b/shoot <player>"));
            return false;
        }
        ...
        target.getWorld().spawnEntity(target.getLocation(), EntityType.DRAGON_FIREBALL);
        return true;
    }
}
